...
class Download(Subcommand):
    ...
    def _download_from_kaggle(self, args: argparse.Namespace) -> None:
        ...
        try:
            ...
            true_output_dir = copytree(
                kaggle_output_path, output_dir, dirs_exist_ok=True
            )
        except Exception as e:
            tb = traceback.format_exc()
            ...
            # stick with the path originally written to by kagglehub
            true_output_dir = kaggle_output_path

        # since kagglehub may output a single filepath, normalize this to a list of files
        if os.path.isdir(true_output_dir):
            output_list = list(Path(true_output_dir).iterdir())
        else:
            output_list = [true_output_dir]
